...
![High-level architecture for Archetype showing browsers flowing through Nginx into the Next.js frontend and Django API, which orchestrate Postgres, Meilisearch, Redis, Celery and the SIPI IIIF image server, all managed by Docker Compose.](../../assets/archetype-architecture.svg)
...
| Data services | Persistent storage, full-text search, media delivery | Postgres, Meilisearch, Redis (optional), SIPI IIIF image server |
...
- **Datastores:** Postgres remains the source of truth for manuscripts, people, places, hands, annotations, and taxonomy metadata. Meilisearch powers deep filtering and fuzzy matching. SIPI streams IIIF-compliant imagery so we can display tiles and annotations without custom servers.
...
3. Search queries hit Meilisearch through the API layer, which applies permissions and aggregates.
